...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_hpo", type=str, default="false")
    ...
    # load datasets
    dataset1 = load_dataset(args.dataset_id, args.dataset_config,split="train[:75%]")
    dataset2 = load_dataset(args.dataset_id, args.dataset_config,split="train[75%:]")

    # process dataset
    def process(examples):
        tokenized_inputs = tokenizer(examples["sms"], truncation=True, max_length=512)
        return tokenized_inputs
    
    tokenized_datasets={}
                            
    tokenized_datasets["train"] = dataset1.map(process, batched=True)
    tokenized_datasets["train"] = tokenized_datasets["train"].rename_column("label", "labels")
    
    tokenized_datasets["validation"] = dataset2.map(process, batched=True)
